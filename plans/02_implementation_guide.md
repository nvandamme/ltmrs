...
- [x] Capture and test `memory_add.confirm` verbatim-storage behavior. Preserve it in the approved legacy policy, or reject explicitly and record the stricter policy deviation; never silently transform confirmed content.

- [x] Reuse frozen input/output schemas and implement explicit legacy DTO conversion.
- [x] Match absent/null/default/enum/case-sensitive fields and unknown-field handling from the actual contract.
- [x] Match structured/text results, `response_format`, explain output, pagination and error classes.
- [x] Reproduce observable confidence/access/context tagging, deduplication, consolidation, invalidation and archival behavior.
- [x] Add native enhancements only through separately negotiated configuration or additional native tools, not new required legacy arguments.
- [x] Implement instructions and dynamic tool-description context scoped to the frontend's project/channel.
- [x] Snapshot static schemas separately from generated dynamic context; test appropriate notification behavior.
- [x] Write differential upstream traces for each tool and approved retrieval deviations. (Automated differential harness: anonymized 177-fragment fixture derived from the real upstream DB, reference output generated by running the pinned upstream code; `differential_detail/summary_matches_upstream_wire` assert byte-for-byte equivalence. Caught + fixed 3 wire-contract bugs: relation-target UUID vs legacy_id, missing Refined from/into, created date format.)
